...
def load_cifar(args):
    ...
    # Create validation loader from last 20% of training data
    if args.dataset == 'cifar10':
    ...
    val_idx = list(range(num_train - split, num_train))
    val_data = [full_train_dataset[i][0].numpy() for i in val_idx]
    val_labels = [full_train_dataset[i][1] for i in val_idx]

    val_data = np.array(val_data)
    val_labels = np.array(val_labels)

    val_dataset = DatasetClass(val_data, torch.zeros_like(val_labels).float(), torch.from_numpy(val_labels).float(),
                               test_transform)
    ...
    return partial_matrix_train_loader, val_loader, partialY, test_loader, est_loader, init_label_dist, train_label_cnt

...
parser.add_argument('--no_imbalance', action='store_true',
                    help='flag to skip dataset imbalancing')

# os.environ['CUDA_VISIBLE_DEVICES'] = '0'

def save_scores_to_csv(predictions, true_labels, filename):
    with open(filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        # Write the header with label names
        writer.writerow(['True Label'] + [f'Label_{i}' for i in range(len(predictions[0]))])
        for pred, label in zip(predictions, true_labels):
            writer.writerow([label] + pred.tolist())

class Trainer():
    def __init__(self, args):
        global train_loader, val_loader, test_loader, train_givenY, train_label_cnt
        self.args = args
        ...
        if args.dataset == 'cifar10':
            ...
            many_shot_num = 4
            low_shot_num = 3
            train_loader, val_loader, test_loader, est_loader, init_label_dist, train_label_cnt, train_givenY = load_cifar(args=args)
    ...
    def train(self):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            # Validation inference and saving scores
            val_predictions, val_true_labels = self.get_predictions(model, self.val_loader)
            save_scores_to_csv(val_predictions, val_true_labels, os.path.join(args.exp_dir, f'validation_scores_epoch_{epoch}.csv'))

            # Test inference and saving scores
            test_predictions, test_true_labels = self.get_predictions(model, self.test_loader)
            save_scores_to_csv(test_predictions, test_true_labels, os.path.join(args.exp_dir, f'test_scores_epoch_{epoch}.csv'))
            ...
            with open(os.path.join(args.exp_dir, 'result.log'), 'a+') as f:
                f.write(
                    'Epoch {}/{}: Val_Acc_tail {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_tail, acc_many_tail, acc_med_tail, acc_few_tail,
                        optimizer.param_groups[0]['lr']))
                f.write(
                    'Epoch {}/{}: Val_Acc_head {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_head, acc_many_head, acc_med_head, acc_few_head,
                        optimizer.param_groups[0]['lr']))
                f.write(
                    'Epoch {}/{}: Val_Acc_ens {:.2f}, Best Val_Acc_ens {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_ens, best_acc_ens, acc_many_ens, acc_med_ens, acc_few_ens,
                        optimizer.param_groups[0]['lr']))
                f.write(
                    'Epoch {}/{}: Test_Acc_tail {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_tail, acc_many_tail, acc_med_tail, acc_few_tail,
                        optimizer.param_groups[0]['lr']))
                f.write(
                    'Epoch {}/{}: Test_Acc_head {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_head, acc_many_head, acc_med_head, acc_few_head,
                        optimizer.param_groups[0]['lr']))
                f.write(
                    'Epoch {}/{}: Test_Acc_ens {:.2f}, Best Test_Acc_ens {:.2f}, Shot - Many {:.2f}/ Med {:.2f}/Few {:.2f}. (lr {:.5f})\n'.format(
                        epoch, args.epochs, acc_test_ens, best_acc_ens, acc_many_ens, acc_med_ens, acc_few_ens,
                        optimizer.param_groups[0]['lr']))
        ...
        # Test inference and saving scores at the end
        test_predictions, test_true_labels = self.get_predictions(model, self.test_loader)
        save_scores_to_csv(test_predictions, test_true_labels, os.path.join(args.exp_dir, 'final_test_scores.csv'))
    ...
    def get_loss(self, X_w, logits_w, logits_s, ce_label, Y, index, model, loss_fn, emp_dist, alpha, eta, epoch, is_tail):
        bs = X_w.shape[0]
...
##                # Print the type and shape of outputs in the first iteration
##                if i == 0:
##                    if isinstance(outputs, tuple):
##                        print(f"Output is a tuple with {len(outputs)} elements.")
##                        for idx, out in enumerate(outputs):
##                            print(f"Element {idx}: type={type(out)}, shape={out.shape}")
##                    else:
...
##        return predictions, true_labels
    ...
    def get_predictions(self, model, data_loader):
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(data_loader):
                images = images.cuda()
                logit_head, logit_tail, feat = model(images)
                
                # Use the ensemble method to get the final probabilities
                probs = model.ensemble(logit_head, logit_tail, self.loss_fn.get_distribution()).cpu().numpy()

...
# #     #     val_dataset = torch.utils.data.TensorDataset(torch.from_numpy(train_dataset.val_data),
...
from cifar_models import convnet, resnet
from datasets.cifar10_lt__ import cifar10
from utils.utils_loss import partial_loss
...
parser.add_argument('-bs', help='batch size', type=int, default=256)
parser.add_argument('-ep', help='number of epochs', type=int, default=500)
parser.add_argument('-ds', help='specify a dataset', type=str, default='cifar10_lt',
                    choices=['mnist', 'fashion', 'kmnist', 'cifar10', 'cifar100', 'cub200', 'cifar10_lt', 'cifar100_lt'], required=False)
parser.add_argument('-model', help='model name', type=str, default='resnet',
                    choices=['linear', 'mlp', 'convnet', 'resnet', 'resnet_'], required=False)
parser.add_argument('-decaystep', help='learning rate\'s decay step', type=int, default=500)
parser.add_argument('-decayrate', help='learning rate\'s decay rate', type=float, default=1)
parser.add_argument('-partial_type', help='flipping strategy', type=str, default='binomial',
...
def main():
    ...
    #     # ])
    #     val_dataset = torch.utils.data.TensorDataset(
    #         #train_dataset.val_data.permute(0, 3, 1, 2).float(),  # Transpose to [3, 32, 32]
    #         torch.tensor(train_dataset.val_data, dtype=torch.long),
    #         torch.tensor(train_dataset.val_labels, dtype=torch.long),
    #         torch.arange(len(train_dataset.val_labels)))
    #     val_loader = torch.utils.data.DataLoader(dataset=val_dataset,
    #                                              batch_size=args.bs,
    ...
    #         torch.tensor(train_dataset.val_data).float(),  # Ensure data is float for tensor operations
    #         torch.tensor(train_dataset.val_labels, dtype=torch.long)  # Labels should be long type
    #     )
    ...
    if hasattr(train_dataset, 'val_data') and train_dataset.validation_split:
        val_dataset = TensorDataset(
            torch.tensor(train_dataset.val_data, dtype=torch.float32),
            torch.tensor(train_dataset.val_labels, dtype=torch.long)
        )
        val_loader = DataLoader(dataset=val_dataset,
                                batch_size=args.bs,
    ...
    # if hasattr(train_dataset, 'val_data') and train_dataset.validation_split:
    #     val_dataset = TensorDataset(
    #         torch.tensor(train_dataset.val_data).permute(0, 3, 1, 2).float(),  # Ensure data is in the correct shape
    #         torch.tensor(train_dataset.val_labels, dtype=torch.long)  # Labels should be long type
    #     )
    #     val_loader = DataLoader(dataset=val_dataset,
    #                             batch_size=args.bs,

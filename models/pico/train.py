...
def main():
    ...
    model_path = 'ds_{ds}_pr_{pr}_lr_{lr}_ep_{ep}_ps_{ps}_lw_{lw}_pm_{pm}_arch_{arch}_sd_{seed}'.format(
        ds=args.dataset,
...
def test_dataloader(loader):
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(loader):
        print(f"Batch {i}, Indexes: {index}")
##########################################################################################
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'cub200':
        ...
        train_loader, train_givenY, train_sampler, test_loader, val_loader = load_cub200(input_size=input_size
                                                                                         ,
                                                                                         partial_rate=args.partial_rate,
                                                                                         batch_size=args.batch_size)
    elif args.dataset == 'cifar10':
        train_loader, train_givenY, train_sampler, test_loader, val_loader = load_cifar10(
...
##        print("train loader shape", train_loader.shape)
        ...
        train_loader, train_givenY, train_sampler, test_loader, val_loader = load_cifar100(
            partial_rate=args.partial_rate, batch_size=args.batch_size, hierarchical=args.hierarchical)
        #train_loader, train_givenY, train_sampler, test_loader, val_loader = load_cifar100(
            #partial_rate=args.partial_rate, batch_size=args.batch_size)#, hierarchical=args.hierarchical)
        #test_dataloader(train_loader)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # acc_val = test(model, val_loader, args, epoch, logger)
        acc_test, test_scores, test_labels = test(model, test_loader, args, epoch, logger)
        acc_val, val_scores, val_labels = test(model, val_loader, args, epoch, logger)
        ...
        with open(os.path.join(args.exp_dir, 'result.log'), 'a+') as f:
            f.write('Epoch {}: Acc Test {}, Acc Val {}, Best Acc {}, lr {}, MMC {}\n'.format(
                epoch, acc_test, acc_val, best_acc, optimizer.param_groups[0]['lr'], mmc))
        ...
        # Save final scores and labels to CSV after the last epoch
    df_test = pd.DataFrame({
        'Scores': [list(score) for score in all_test_scores],
        'True Label': all_test_labels
    })
    df_test.to_csv(os.path.join(args.exp_dir, 'final_test_results.csv'), index=False)

    df_val = pd.DataFrame({
        'Scores': [list(score) for score in all_val_scores],
        'True Label': all_val_labels
    })
    df_val.to_csv(os.path.join(args.exp_dir, 'final_val_results.csv'), index=False)
...
def test(model, test_loader, args, epoch, tb_logger):
    ...
    return acc_tensors[0], all_scores, all_labels

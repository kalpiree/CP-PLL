...
class cifar10(data.Dataset):
    ...
    def __init__(self, root, train_or_not=True, val_or_not=False, download=False, transform=None, target_transform=None,
                 imb_factor=0.01, imbalance=True):
        ...
        if self.train:
            ...
            print(f"After loading: {len(self.train_data)} images, {len(self.train_labels)} labels")

            if self.imbalance:
                self.train_data, self.train_labels = self.gen_imbalanced_data(self.train_data, self.train_labels, 10,
                                                                              'exp',
                                                                              self.imb_factor)
                print(f"After imbalance: {len(self.train_data)} images, {len(self.train_labels)} labels")
            self.train_data = torch.from_numpy(self.train_data).byte()
            self.train_labels = torch.tensor(self.train_labels, dtype=torch.long)
            print(f"After conversion to tensors: {len(self.train_data)} images, {len(self.train_labels)} labels")
    ...
    def gen_imbalanced_data(self, data, targets, num_classes, imb_type='exp', imb_factor=0.01):
        ...
        if len(classes) != num_classes:
            raise ValueError(f"Expected {num_classes} classes, but found {len(classes)}")

        for the_class, the_img_num in zip(classes, img_num_per_cls):
            idx = np.where(targets_np == the_class)[0]
            np.random.shuffle(idx)
            if len(idx) < the_img_num:
                print(f"Warning: Not enough images for class {the_class}. Reducing from {the_img_num} to {len(idx)}.")
                the_img_num = len(idx)
            
            selec_idx = idx[:the_img_num]

            # Ensure selec_idx has the correct length
            if len(selec_idx) != the_img_num:
                raise ValueError(f"Expected {the_img_num} images for class {the_class}, but got {len(selec_idx)}")
            #selec_idx = idx[:the_img_num]
            # Ensure selec_idx has the correct length
            #if len(selec_idx) != the_img_num:
            #    raise ValueError(f"Expected {the_img_num} images for class {the_class}, but got {len(selec_idx)}")
    ...
    # Ensure the number of images and labels match
        if len(new_data) != len(new_targets):
            raise ValueError(f"Number of images ({len(new_data)}) does not match number of labels ({len(new_targets)}) after imbalance.")

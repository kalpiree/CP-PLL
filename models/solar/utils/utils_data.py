...
from .cifar100_lt import cifar100
from .cifar10_lt import cifar10
...
def load_cifar(args):
    global test_dataset, val_dataset
    ...
    data_file = "{ds}_{pr}_imb_{it}{imf}_sd{sd}.npy".format(
        ds=args.dataset,
        pr=args.partial_rate,
        it=args.imb_type if args.imbalanced else 'none',
        imf=args.imb_ratio if args.imbalanced else 1.0,
        sd=args.seed)
    ...
    return partial_matrix_train_loader, partialY, test_loader, val_loader, est_loader, init_label_dist, train_label_cnt
...
def get_train_data(dataset):
    ...
    if dataset == 'cifar10':
        ...
        temp_train = cifar100(root='./cifar10_lt/',
                              download=True,
                              train_or_not=True,
                              val_or_not=False,
                              transform=transforms.Compose([transforms.ToTensor(),
    ...
    data, labels = np.array(temp_train.train_data), np.array(temp_train.train_labels)
    return data, labels
...
def generate_imb_pll_data(args):
    ...
    # Ensure train_labels is a PyTorch tensor before passing to the function

...
parser.add_argument('--imbalanced', action='store_true', help='Generate imbalanced data')
...
class Trainer():
    def __init__(self, args):
        ...
        model_path = '{ds}_{pr}_ql{ql}_rho{rho}_gm{gm}_t{t}_ep{we}_{ee}'.format(
            ds=args.dataset,
            pr=args.partial_rate,
            ep=args.epochs,
            ql=args.queue_length,
        ...
        if args.imbalanced:
            model_path += '_imb_{it}{imf}_sd_{seed}'.format(
                it=args.imb_type,
                imf=args.imb_factor,
                seed=args.seed)
        else:
            model_path += '_balanced_sd_{seed}'.format(seed=args.seed)
        ...
        if args.dataset == 'cifar10':
            train_loader, train_givenY, test_loader, val_loader, est_loader, init_label_dist, train_label_cnt\
            ...
            input_size = 224
            train_loader, train_givenY, test_loader, val_loader, est_loader, init_label_dist, train_label_cnt = load_cub200(
            ...
            input_size = 224
            train_loader, train_givenY, test_loader, est_loader, val_loader, init_label_dist, train_label_cnt = load_sun397(
                                                                            data_dir=args.data_dir,
                                                                            input_size=input_size, 
                                                                            partial_rate=args.partial_rate, 
                                                                            batch_size=args.batch_size)
    ...
    def train(self, emp_dist=None, is_est_dist=False, total_epochs=0, gamma=0):
        ...
        for epoch in range(total_epochs):
            ...
            # acc_val, acc_many, acc_med, acc_few = self.test(model, self.test_loader)
            acc_test, acc_many, acc_med, acc_few, test_scores, test_labels = self.test(model, self.test_loader)
            acc_val, acc_many, acc_med, acc_few, val_scores, val_labels = self.test(model, self.val_loader)
            ...
            with open(os.path.join(args.exp_dir, 'result.log'), 'a+') as f:
                ...
                f.write('Epoch {}: Acc Val {:.2f}\n'.format(epoch, acc_val))
            ...
            # save checkpoints
            # Save final scores and labels to CSV
        test_df = pd.DataFrame({
            'Scores': [list(score) for score in test_scores],
            'True Label': test_labels
        })
        val_df = pd.DataFrame({
            'Scores': [list(score) for score in val_scores],
            'True Label': val_labels
        })

        test_df.to_csv(os.path.join(args.exp_dir, 'test_results.csv'), index=False)
        val_df.to_csv(os.path.join(args.exp_dir, 'val_results.csv'), index=False)
